    ...
    <div class='order-list' v-for="(item,index) in orders" :key="index" @click="$router.push('/user/order/details?ordernum='+item.ordernum)">
      ...
      <div class='total-wrap'>
        ...
        <div class="status-wrap">
          <div class='status-btn' v-if="item.status==='0'" @click="cancelOrder(index,item.ordernum)">取消订单</div>
          <div class='status-btn' @click="sureOrder(index,item)">{{item.status=='0'?'去付款':item.status=='1'?'确认收货':'已收货'}}</div>
        </div>
      </div>
    </div>
  </div>
