...
export default {
  ...
  mutations: {
        ...
        for (let i = 0; i < state.cartData.length; i++) {
          if (
            ...
            state.cartData[i].amount =
              parseInt(state.cartData[i].amount) +
    ...
    ["SET_AMOUNT"](state, payload) {
      ...
      state.cartData[payload.index].amount = parseInt(
        state.cartData[payload.index].amount.replace(/[^\d]/g, "")
      );
    ...
    ["SELECT_ITEM"](state, payload) {
      state.cartData[payload.index].checked = !state.cartData[payload.index]
        .checked;
      Vue.set(state.cartData, payload.index, state.cartData[payload.index]);
      localStorage["cartData"] = JSON.stringify(state.cartData);
    },
